...
mohp2= Multiorderhighpass(3,loop_frequency,20,1)    #Filter order, sampling frequency, cutoff frequency, Static Gain
rmsf2= Rmsfilter(50)    #Number of processed values
...
def loop():
 ...
 # Read value of 16bit ADC between 0-65535 corresponding to 0V-3.3V
    if (rmsf.process(abs(mohp.process(adc.read_u16())))/calibration_result) > 0.1:
        result_1= 1
    else:
        result_1 =0
    
    if (rmsf2.process(abs(mohp2.process(adc2.read_u16())))/calibration_result) > 0.1:
        result_2= 1
    else:
        result_2 =0
    #pc.set(0, adc.read_u16())
    pc.set(0,mohp.process(adc.read_u16()))
    pc.set(1,rmsf.process(abs(mohp.process(adc.read_u16()))))
    pc.set(2,result_1)
    pc.set(3,mohp2.process(adc2.read_u16()))
    pc.set(4,rmsf2.process(abs(mohp2.process(adc2.read_u16()))))
    pc.set(5,result_2)
