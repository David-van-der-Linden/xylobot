...
class EmgInput():
    def __init__(self,loop_frequency,filter_order,calibration_result_left,calibration_result_right,cutoff_frequency,rmsfilter_window_size):
        ...
        self.mohp1= Multiorderhighpass(filter_order,loop_frequency,cutoff_frequency,1)    #Filter order, sampling frequency, cutoff frequency, Static Gain
        self.mohp2= Multiorderhighpass(filter_order,loop_frequency,cutoff_frequency,1)    #Filter order, sampling frequency, cutoff frequency, Static Gain
    ...
    def getEmgLeft(self): # This function needs to be called with the loop_frequency 
        ...
        # Read value of 16bit ADC between 0-65535 corresponding to 0V-3.3V
        if (self.rmsf2.process(abs(self.mohp2.process(adc.read_u16())))/self.calibration_result_left) > 0.1:
            return True
        return False
    ...
    def getEmgRight(self): # This function needs to be called with the loop_frequency 
        adc = ADC(Pin('A1'))
        # Read value of 16bit ADC between 0-65535 corresponding to 0V-3.3V
        if (self.rmsf2.process(abs(self.mohp2.process(adc.read_u16())))/self.calibration_result_right) > 0.1:
            return True
        return False
